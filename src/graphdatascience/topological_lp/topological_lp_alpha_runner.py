...
class TopologicalLPAlphaRunner(UncallableNamespace, IllegalAttrChecker):
    ...
    def _run_standard_function(self, node1: int, node2: int, config: dict[str, Any]) -> float:
        ...
        return self._query_runner.run_cypher(query, QueryType.USER_TRANSPILED, params)["score"].squeeze()  # type: ignore
    ...
    def sameCommunity(self, node1: int, node2: int, communityProperty: str | None = None) -> float:
        ...
        """
        return self._query_runner.run_cypher(query, QueryType.USER_TRANSPILED)["score"].squeeze()  # type: ignore
