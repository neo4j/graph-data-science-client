...
class SimilarityAlgoRunner(IllegalAttrChecker):
    ...
    def _run_procedure(self, config: Dict[str, Any]) -> Row:
        return self._query_runner.run_query(f"CALL {self._namespace}($config)", {"config": config})[0]

    def __call__(self, vector1: List[float], vector2: List[float]) -> float:
        result = self._query_runner.run_query(f"RETURN {self._namespace}({vector1}, {vector2}) AS similarity")
