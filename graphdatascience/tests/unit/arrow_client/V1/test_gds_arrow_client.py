...
def test_create_graph_with_defaults(flight_server: FlightServer, gds_client: GdsArrowClient) -> None:
    gds_client.create_graph("g", "DB")
    actions = flight_server._actions
    assert len(actions) == 1
    assert_action(actions[0], "v1/CREATE_GRAPH", {"name": "g", "database_name": "DB"})


def test_create_graph_with_options(flight_server: FlightServer, gds_client: GdsArrowClient) -> None:
    gds_client.create_graph(
...
def test_create_graph_with_flaky_server(
    flaky_flight_server: FlakyFlightServer, flaky_gds_client: GdsArrowClient
) -> None:
    flaky_gds_client.create_graph("g", "DB")
    actions = flaky_flight_server._actions
    assert len(actions) == flaky_flight_server.expected_retries()
    assert_action(actions[0], "v1/CREATE_GRAPH", {"name": "g", "database_name": "DB"})


def test_create_graph_from_triplets_with_defaults(flight_server: FlightServer, gds_client: GdsArrowClient) -> None:
    gds_client.create_graph_from_triplets("g", "DB")
    actions = flight_server._actions
    assert len(actions) == 1
    assert_action(actions[0], "v1/CREATE_GRAPH_FROM_TRIPLETS", {"name": "g", "database_name": "DB"})


def test_create_graph_from_triplets_with_options(flight_server: FlightServer, gds_client: GdsArrowClient) -> None:
    gds_client.create_graph_from_triplets(
        "g", "DB", undirected_relationship_types=["Foo"], inverse_indexed_relationship_types=["Bar"], concurrency=42
    )
    actions = flight_server._actions
    assert len(actions) == 1
...
def test_create_database_with_defaults(flight_server: FlightServer, gds_client: GdsArrowClient) -> None:
    gds_client.create_database("g")
    actions = flight_server._actions
    assert len(actions) == 1
    assert_action(
...
def test_create_database_with_options(flight_server: FlightServer, gds_client: GdsArrowClient) -> None:
    gds_client.create_database(
        "g",
        "DB",
        id_property="foo",
        db_format="BLOCK",
...
def test_node_load_done_action(flight_server: FlightServer, gds_client: GdsArrowClient) -> None:
    response = gds_client.node_load_done("g")
...
def test_relationship_load_done_action(flight_server: FlightServer, gds_client: GdsArrowClient) -> None:
    response = gds_client.relationship_load_done("g")
    assert response.name == "g"
...
def test_triplet_load_done_action(flight_server: FlightServer, gds_client: GdsArrowClient) -> None:
    response = gds_client.triplet_load_done("g")
    assert response.name == "g"
    assert response.node_count == 42
...
def test_abort_action(flight_server: FlightServer, gds_client: GdsArrowClient) -> None:
    gds_client.abort("g")
    actions = flight_server._actions
    assert len(actions) == 1
...
def test_get_node_property(flight_server: FlightServer, gds_client: GdsArrowClient) -> None:
    gds_client.get_node_properties("g", "db", "id", ["Person"], concurrency=42)
...
def test_flakey_get_node_property(flaky_flight_server: FlakyFlightServer, flaky_gds_client: GdsArrowClient) -> None:
    # differs to previous GdsArrowClient to not retry get streams
    with pytest.raises(FlightTimedOutError):
        flaky_gds_client.get_node_properties("g", "db", "id", ["Person"], concurrency=42)


def test_get_node_properties(flight_server: FlightServer, gds_client: GdsArrowClient) -> None:
    gds_client.get_node_properties("g", "db", ["foo", "bar"], ["Person"], list_node_labels=True, concurrency=42)
    tickets = flight_server._tickets
    assert len(tickets) == 1
    assert_ticket(
        tickets[0],
...
def test_get_node_labels(flight_server: FlightServer, gds_client: GdsArrowClient) -> None:
    gds_client.get_node_labels("g", "db", concurrency=42)
    tickets = flight_server._tickets
    assert len(tickets) == 1
    assert_ticket(
        tickets[0],
...
def test_get_relationship_property(flight_server: FlightServer, gds_client: GdsArrowClient) -> None:
    gds_client.get_relationship_properties(
...
def test_get_relationship_properties(flight_server: FlightServer, gds_client: GdsArrowClient) -> None:
    gds_client.get_relationship_properties(
...
def test_get_relationship_topologys(flight_server: FlightServer, gds_client: GdsArrowClient) -> None:
    gds_client.get_relationships("g", "db", relationship_types=["FOO"], concurrency=42)
    tickets = flight_server._tickets
    assert len(tickets) == 1
    assert_ticket(
        tickets[0],
...
def test_serialize_roundtrip(flight_server: FlightServer, gds_client: GdsArrowClient) -> None:
    import pickle

    pickled_client = pickle.dumps(gds_client)
    unpickled_client = pickle.loads(pickled_client)
    assert isinstance(unpickled_client, GdsArrowClient)
    assert unpickled_client._flight_client is not None
