...
For example if one needs to configure the Neo4j driver that is used in a certain a certain way.
...
NOTE: You can also use `gds.graph.load_cora` to get started with a real dataset. See xref:graph-object.adoc#graph-object-load-cora[this segment] for more details.
