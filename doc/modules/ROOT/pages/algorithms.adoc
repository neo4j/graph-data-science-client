...
include::partial$/python-client-syntax.adoc[]
...
The result of running a procedure is returned as either a pandas `DataFrame` or a pandas `Series` depending on the xref:algorithms.adoc#python-client-algorithms-execution-mode[execution mode].
