...
Once created, the `TrainingPipeline` can be passed as arguments to methods in the Python client, such as the https://neo4j.com/docs/graph-data-science/current/pipeline-catalog/pipeline-catalog/pipeline-catalog/[pipeline catalog operations].
...
result = trained_pipe_model.predict_stream(H, concurrency=2)
...
# Train the pipeline targeting node property "my-target" and "MEAN_SQUARED_ERROR" as only metric
...
[source,python]
----
exists_result = gds.beta.pipeline.exists(pipe)
