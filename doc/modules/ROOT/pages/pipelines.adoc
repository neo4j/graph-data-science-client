...
[source,python,role=test,group=nc-pipe]
----
gds.run_cypher(
  """
  CREATE 
...
assert metrics["ACCURACY"]["test"] >= 0.5
...
predictions = fraud_model.predict_stream(H, concurrency=8)
...
[source,python,role=test,group=lp-pipe]
----
gds.run_cypher(
  """
  CREATE 
...
// FIXME enable this test again when not flaky anymore
...
[source,python,role=test,group=nr-pipe]
----
gds.run_cypher(
  """
  CREATE 
...
assert round(metrics["MEAN_SQUARED_ERROR"]["test"], 2) <= 254.30, metrics["MEAN_SQUARED_ERROR"]["test"]

H, project_result = gds.graph.project("full_person_graph", ["Person", "UnknownPerson"], "KNOWS")

# Predict on `H` and stream the results with a specific concurrency of 8
predictions = age_predictor.predict_stream(H, concurrency=8)
assert len(predictions) == H.node_count()
----
