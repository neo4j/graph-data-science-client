...
The method that exposes the NetworkX dataset loading functionality is called `gds.graph.datasets.networkx.load`.
...
Exactly how the `networkx.Graph` `nx_G` maps to a GDS `Graph` projection is outlined in detail xref:load-from-networkx.adoc#nx-schema[below].
