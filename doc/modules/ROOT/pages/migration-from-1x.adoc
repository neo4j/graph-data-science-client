...
This is sufficient for most endpoints. Graph projections are the exception: the projection endpoints change shape starting with the 2.0 release, so projection code must be updated as described in <<graph-projections,Graph projections>>, even if you're already on `gds.v2`.
