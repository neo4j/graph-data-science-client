...
The property streaming endpoints keep their 1.x behaviour, but the plural endpoints have absorbed the ones that streamed properties for a specific relationship type:
...
* `gds.graph.relationshipProperty.stream` is now `gds.graph.relationship_property.stream`.
* `gds.graph.relationshipProperties.stream` is now `gds.graph.relationships.stream`, which takes the properties to stream as a `relationship_properties` parameter.
* The client-only `separate_property_columns` parameter is gone.
  The plural endpoints always return one column per requested property; the singular endpoints always return the values in a single `propertyValue` column.
...
[source, python]
----
df = gds.graph.relationships.stream(
    G, relationship_properties=["weight"]
)
----
|===
