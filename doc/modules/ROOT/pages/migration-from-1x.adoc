...
Cypher queries run via `run_cypher` now always use transactional retries by default.

Queries using `CALL { ... } IN TRANSACTIONS` are not supported by these driver-managed transactions.
Use the `auto_commit` parameter to run such a query in an https://neo4j.com/docs/python-manual/current/query-advanced/#implicit-transactions[auto-commit transaction] instead:
