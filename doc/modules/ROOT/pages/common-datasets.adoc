...
`IS_FRIEND` which represents User-User friendship.

`LISTEN_TO` between User and Artist.
It has a relationship property `weight` that represents the number of times the user listened to the artist.

`TAGGED` between User and Artist.
It has a `tagID` property where each `tagId` represents a genre,
...
Note that a user can tag the same artist at different times with different `tagId`.
