...
* `gds.graph.datasets.ogbn.load` for loading https://ogb.stanford.edu/docs/nodeprop/[node property prediction datasets], and
* `gds.graph.datasets.ogbl.load` for loading https://ogb.stanford.edu/docs/linkprop/[link property prediction datasets].
...
| concurrency       | int               | An optional number of threads to use. In Aura Graph Analytics, the default is the number of available processors. Otherwise, the default 4.
|===
...
If you want a projected graph to be `UNDIRECTED` in the Neo4j GDS sense of the word, you can modify it with link:{gds-docs-base-uri}/graph-catalog-relationship-ops/#catalog-graph-relationship-to-undirected-example[`gds.graph.relationships.toUndirected`].
====

[WARNING]
====
If the targeted GDS server has an enterprise license and the link:{gds-docs-base-uri}/installation/installation-apache-arrow/[Arrow Flight Server] enabled, the performance of the OGB dataset loading will be very good.
...
----
G = gds.graph.datasets.ogbn.load("ogbn-arxiv")
...
[source, python, role=no-test]
----
G = gds.graph.datasets.ogbn.load("ogbn-mag")
...
[source, python, role=no-test]
----
G = gds.graph.datasets.ogbl.load("ogbl-ddi")
...
[source, python, role=no-test]
----
G = gds.graph.datasets.ogbl.load("ogbl-biokg")
