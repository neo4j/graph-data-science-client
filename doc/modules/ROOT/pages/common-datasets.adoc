...
The default is to load all tagging relationships as one `TAGGED` relationship type with `hetero_tag_rel = False`.
If `hetero_tag_rel = True`, each `TAGGED` relationship will become a `TAGGED_{tagID}` relationship, encoding tags with different ids as different relationship types.
There are 9749 different tagIDs resulting in 9749 different relationship types.
