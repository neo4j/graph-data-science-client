...
Unlike `gds.run_cypher` but very much like `gds.graph.project.native`, it returns a tuple of a `Graph` object and a `GraphCypherProjectResult` object containing metadata from the Cypher execution, with fields accessed as attributes (for example `result.node_count`).
...
| drop                    | fail_if_missing: Optional[bool] | GraphInfo              | https://neo4j.com/docs/graph-data-science/current/graph-drop/[Removes the graph] from the GDS Graph Catalog.
...
assert not any(info.graph_name == "tmp_offices" for info in gds.graph.list())
----
...
assert any(info.graph_name == "offices" for info in gds.graph.list())
...
The type returned from the Python client method corresponding to https://neo4j.com/docs/graph-data-science/current/graph-catalog-relationship-ops/#_stream[`gds.graph.relationships.stream`] is called `RelationshipsDataFrame` and inherits from the standard pandas `DataFrame`.
