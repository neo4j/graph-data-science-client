...
Cypher projection is not a dediicated procedure; rather it requires writing a Cypher query that calls the `gds.graph.project` aggregation.
...
The method `gds.graph.cypher.project` bridges the gap between <<getting-started-minimal-example, `gds.run_cypher`>> and `gds.graph.get`.
It takes as parameters a query string `query: str`, an optional string `database: Optional[str]` to override which database to target as well as an optional Cypher parameters dictionary as keywords args `**params: Any`.
...
The method does not modify the Cypher query in any, all projection configuration must be done in the query itself.
