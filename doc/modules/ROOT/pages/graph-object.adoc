...
Nodes having the same values for the `labels` attribute must have the same set of node property keys.
...
That includes the limitation of having at most one property per relationship.
...
Edges having the same values for the `relationshipType` attribute must have the same set of edge property keys.
