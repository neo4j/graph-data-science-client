...
Before projecting a graph, it is useful to estimate its memory requirements to correctly size a server or an Aura Graph Analytics session.
...
It requires writing a Cypher query that calls the `gds.graph.project` aggregation function, and it is equivalent to running `gds.run_cypher` followed by `gds.graph.get`.
