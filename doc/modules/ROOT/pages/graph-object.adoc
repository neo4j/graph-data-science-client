...
The graph object also implement the context managment protocol, i.e., is usable inside `with` cluases.
This allows automatically dropping the graph at the end of `with` block.

[source,python, graph-project]
----
