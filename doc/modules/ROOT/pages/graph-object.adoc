...
[source,python,role=test,group=graph-project]
----
# Suppose we have this simple graph in our database
gds.run_cypher(
  """
  CREATE
  (m: City {name: "Malmö"}),
...
assert G.node_count() == res["nodeCount"]
----
...
[source,python,role=test,group=graph-project]
----
res = gds.degree.mutate(G, mutateProperty="degree")
assert "centralityDistribution" in res
----
...
[source,python,role=test,group=graph-project]
----
res = gds.graph.streamNodeProperty(G, node_properties="degree")
