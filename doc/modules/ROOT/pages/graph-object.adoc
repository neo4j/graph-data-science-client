...
In addition to projecting a graph from the Neo4j database, it is also possible to create in-memory graphs directly from pandas `DataFrame` objects.
...
The above example creates an in-memory graph from two `DataFrame` objects, one for nodes and one for relationships.
