...
TIP: For the Self-managed and Standalone session types, a `cloud_location` parameter is required.
...
| cloud_location              | https://neo4j.com/docs/graph-data-science-client/{docs-version}/api/sessions/cloud_location[CloudLocation]      | yes      | None    | Aura-supported cloud provider and region where the GDS Session will run. The provider must be one of `"gcp"`, `"aws"`, or `"azure"`, and the region must be an Aura-supported region (e.g. `"europe-west1"`). Required for the Self-managed and Standalone types. Use `sessions.available_cloud_locations()` to list all valid provider/region combinations.
...
| neo4j_driver_config         | dict[str, any]     | yes      | None    | Additional options passed to the Neo4j driver to the Neo4j DBMS. Only relevant if `db_connection` is specified.
...
| show_progress               | bool               | yes      | True    | Whether the returned client should print its own job-progress bars (projection, algorithm execution, ...).
|===
...
.Estimating the size of a GDS Session for individual algorithms:
[source, python, role=no-test]
----
memory = sessions.estimate(
    node_count=20,
...
Algorithm configuration affects the estimate as well, so you can also pass a numeric configuration per algorithm.
Algorithms without configuration map will use default values.

.Estimating the size of a GDS Session for individual algorithms and their configuration:
[source, python, role=no-test]
----
memory = sessions.estimate(
    node_count=20,
...
The following query creates an example graph in the Neo4j database, used by the native projection example below.

[source, python, session=true, group=aga-native-projection]
----
gds.run_cypher(
    """
    CREATE
      (anne: Person {name: "Anne", age: 30}),
      (keanu: Person {name: "Keanu", age: 58}),
      (wallE: Movie {title: "Wall-E", rating: 9.2}),
      (matrix: Movie {title: "The Matrix", rating: 9.4}),
...
The following query creates an example graph in the Neo4j database, used by the Cypher projection example below and the examples further down this page.

[source, python, session=true, group=aga-cypher-projection]
----
gds.run_cypher(
    """
    CREATE
...
[source,python,session=true,group=aga-cypher-projection]
----
gds.page_rank.mutate(G, mutate_property="pr")
gds.fast_rp.mutate(G, feature_properties=["pr"], embedding_dimension=2, node_self_influence=0.1, mutate_property="embedding")

# Stream the results back together with the `name` property fetched from the database
gds.graph.node_properties.stream(G, node_properties=["pr", "embedding"], db_node_properties=["name"])
----
