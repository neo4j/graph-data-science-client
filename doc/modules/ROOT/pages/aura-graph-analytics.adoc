...
You can create a remote projection using the `gds.graph.project.cypher()` endpoint with a graph name, a Cypher query, and additional optional parameters.
...
| node_label_filter                  | list[str]      | no       | -       | List of node labels to include in the graph projection.
| relationship_type_filter           | list[str]      | no       | -       | List of relationship types to include in the graph projection.
| node_properties                    | list[str]      | yes      | None    | List of node properties to include in the graph projection.
| relationship_properties            | list[str]      | yes      | None    | List of relationship properties to include in the graph projection.
| job_id                             | str            | yes      | None    | Identifier for the computation.
| concurrency                        | int            | yes      | 4       | Number of concurrent threads to use.
| undirected_relationship_types      | list[str]      | yes      | []      | List of relationship types to treat as undirected.
| inverse_indexed_relationship_types | list[str]      | yes      | []      | List of relationship types to index in both directions.
| batch_size                         | int            | yes      | 10000   | Number of rows to process in each batch when projecting the graph.
| logging                            | bool           | yes      | True    | Whether to log progress during graph projection.
|===
...
| result | ProjectionResult | Statistical data about the projection.
|===
...
[NOTE]
====
When you create a session, the session name must be unique within the project.
====
...
For comprehensive details on how these projections work and when to use them, see the GDS manual on link:link:{gds-docs-base-uri}/aura-graph-analytics/cypher/#_native_projections[Native projections in Aura Graph Analytics].

.Native projection:
[source, role=no-test]
----
gds.v2.graph.project.native(
    graph_name: str,
...
| concurrency                        | int       | yes      | None    | Concurrency to use for building the graph within the session.
| undirected_relationship_types      | list[str] | yes      | None    | List of relationship type names that should be treated as undirected.
| inverse_indexed_relationship_types | list[str] | yes      | None    | List of relationship type names that should be indexed in reverse.
| batch_size                         | int       | yes      | None    | Size of batches transmitted from the DBMS to the session.
| logging                            | bool      | yes      | True    | Whether to log progress during graph projection.
|===
...
| Name   | Type        | Description
| graph  | https://neo4j.com/docs/graph-data-science-client/{docs-version}/api/graph_object[Graph]       | Graph object representing the projected graph.
| result | StoreProjectionResult | Statistical data about the projection.
|===
...
----
projection_handle = gds.graph.project_native_async(
    "people",
    "*",
    "*"
)
