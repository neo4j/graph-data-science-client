...
[source, python, session=true, group=aga-examples]
----
gds.run_cypher(
    """
    CREATE
...
G, result = gds.graph.project.native(
    ...
    node_label_filter=["User", "Product"],
    relationship_type_filter=["KNOWS", "BOUGHT"],
    node_properties=["cost"]
)
----

[NOTE]
Native projections support only numeric node properties.
String properties such as `name` can instead be included when streaming results back, via the `db_node_properties` parameter (see xref:read-graph.adoc#graph-object-streaming-db-properties[Include node properties from Neo4j]).
