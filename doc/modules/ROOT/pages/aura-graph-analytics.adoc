...
You can create a remote projection using the `gds.graph.project.native()` endpoint with a graph name, a Cypher query, and additional optional parameters.
...
| query_parameters                   | dict[str, Any] | yes      | None    | Parameters that will be passed to the Cypher query.
...
Instead, the graph name is provided to the `gds.graph.project.native()` endpoint.
. The `gds.graph.project.remote()` function must be used, instead of the `gds.graph.project.native()` function.

Query parameters can be used to inject dynamic data into the projection query via the `query_parameters` parameter.

For full details and examples on how to write Cypher projection queries, see the link:{neo4j-docs-base-uri}/graph-data-science/current/management-ops/graph-creation/graph-project-cypher-projection/[Cypher projection documentation in the GDS Manual].
...
For comprehensive details on how these projections work and when to use them, see the GDS manual on link:https://neo4j.com/docs/graph-data-science/current/aura-graph-analytics/cypher/#_native_projections[Native projections in Aura Graph Analytics].

.Native projection:
[source, role=no-test]
----
gds.v2.graph.project_native(
    graph_name: str,
