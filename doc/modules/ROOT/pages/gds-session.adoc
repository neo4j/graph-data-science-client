...
TENANT_ID = None # Needs to be specified if your Aura account has multiple tenants

# Create a new GdsSessions object
sessions = GdsSessions(api_credentials=AuraAPICredentials(client_id=CLIENT_ID, client_secret=CLIENT_SECRET, tenant_id=TENANT_ID))
----
