...
[source,python,role=no-test]
----
from graphdatascience.session import SessionMemory, DbmsConnectionInfo
...
Supported values are: `1GB`, `2GB`, `4GB`, `8GB`, `16GB`, `32GB`, `64GB`, `128GB`, `256GB`, `320GB`, `512GB`.


=== Listing GDS Sessions

The `list()` method returns the name and memory of all currently active GDS Sessions.
