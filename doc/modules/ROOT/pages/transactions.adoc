...
=====
When using GDS Sessions, bookmarks can only be set via `gds.set_bookmarks()`; no initial bookmarks can be provided when creating a session via `GdsSessions.get_or_create()`.
