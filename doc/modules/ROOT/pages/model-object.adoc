...
`Model` objects are typically constructed from training a xref:pipelines.adoc[pipeline] or a xref:model-object.adoc#python-client-model-object-graphsage[GraphSAGE model], in which case a reference to the trained model in the form of a `Model` object is returned.
...
include::partial$/python-client-gds-object.adoc[]
...
How to do so for GraphSAGE is xref:model-object.adoc#python-client-model-object-graphsage[described below], and on the xref:pipelines.adoc[Machine learning pipelines] page for pipelines.
...
As exemplified above in xref:model-object.adoc#python-client-model-object-constructing[Constructing a model object], training a GraphSAGE model with the Python client is analogous to https://neo4j.com/docs/graph-data-science/current/machine-learning/node-embeddings/graph-sage/[its Cypher counterpart].

Once trained, in addition to the xref:model-object.adoc#python-client-model-object-inspecting[methods above], the GraphSAGE model object will have the following methods.
