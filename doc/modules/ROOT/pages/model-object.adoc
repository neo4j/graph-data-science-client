...
[source,python,group=gs-model]
----
model, train_result = gds.graph_sage.train(G, model_name="city-representation", feature_properties=["settled"], random_seed=42)
...
To inspect a model that is already present in the model catalog, call the `get` method with its name, which returns a `ModelDetails` object with the model's metadata:

[source,python,group=gs-model]
----
model = gds.model.get("city-representation")

assert model.model_name == "city-representation"
----
...
| drop    | fail_if_missing: Optional[bool] | ModelDetails | https://neo4j.com/docs/graph-data-science/current/model-catalog/drop/[Removes the model] from the GDS Model Catalog.
| details | -                               | ModelDetails | Metadata about the model.
|===

The `details()` method returns a link:{gds-api-uri}/endpoints/model_endpoints/#graphdatascience.model.model_details.ModelDetails[`ModelDetails`] object whose fields are accessed as attributes:

.`ModelDetails` fields
[opts="header",cols="6m,7m,14"]
|===
| Name          | Type                | Description
| model_name    | str                 | The name of the model as it appears in the model catalog.
| model_type    | str                 | The type of model it is, eg. "graphSage".
...
| published     | bool                | `True` if the model is https://neo4j.com/docs/graph-data-science/current/model-catalog/publish/[published] between users, `False` otherwise.
| model_info    | dict[str, Any]      | Additional model-specific information, such as training metrics.
...
| predict_mutate | G: Graph, +
                   config: **kwargs | GraphSageMutateResult  | https://neo4j.com/docs/graph-data-science/current/machine-learning/node-embeddings/graph-sage/#graph-sage-trained-model-example-mutate[Predict embeddings for nodes of the input graph and mutate graph with predictions].
...
| predict_write  | G: Graph, +
                   config: **kwargs | GraphSageWriteResult   | https://neo4j.com/docs/graph-data-science/current/machine-learning/node-embeddings/graph-sage/#graph-sage-trained-model-example-write[Predict embeddings for nodes of the input graph and write the results back to the database].
|===
