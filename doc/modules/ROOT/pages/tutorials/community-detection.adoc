...
----
gds.run_cypher(
    "UNWIND $nodes AS node MERGE (n:Subreddit {name: node.SUBREDDIT})",
...
gds.run_cypher(
    ...
    MERGE (source)-[:HYPERLINKED_TO]->(target)
    """,
...
[source, python, role=no-test]
----
result = gds.wcc.mutate(G, mutate_property="componentId")
...
components = gds.run_cypher(query, retryable=True)
components
----
...
largest_component_graph, _ = gds.graph.filter(
    G, "largest_connected_components", f"n.componentId={largest_component}", "*"
)
...
[source, python, role=no-test]
----
gds.louvain.mutate(largest_component_graph, mutate_property="louvainCommunityId")
----
...
[source, python, role=no-test]
----
gds.graph.node_properties.write(largest_component_graph, ["louvainCommunityId"])
----
...
communities = gds.run_cypher(query, retryable=True)
communities
----
...
----
# Cleanup database
gds.run_cypher("MATCH (n:Subreddit) DETACH DELETE n", retryable=True)
----
