...
version `+2.0a1+` or later, as well as `+pyspark+`.
...
like `+undirected_relationship_types+`. It returns a job id, that we
need to reference the import job in the following steps.
...
module and adding `+time.sleep(1)+` inside the `+while+` loop at the end
of the cell.
...
def retrieve_data(ignored):
    stream_data = arrow_client.stream_job(job_id)
