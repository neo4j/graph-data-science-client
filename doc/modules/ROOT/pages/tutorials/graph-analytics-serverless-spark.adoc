...
----
%pip install "graphdatascience>=1.22" python-dotenv "pyspark[sql]"
----
...
https://spark.apache.org/docs/latest/api/python/getting++_++started/
...
like `undirected++_++relationship++_++types`. It returns a job id, that
we need to reference the import job in the following steps.
