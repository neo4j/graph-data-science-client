...
self managed Neo4j database instance has been set up and is available to
access. We need to provide the database Bolt address, username, and
password to the `DbmsConnectionInfo` class.
...
`people-and-fruits-sm`. If we had already created a session and want to
reconnect to it, the same code is used. Doing that will not incur any
additional costs, and will be a lot faster.
...
# Identify the Neo4j database instance
db_address = os.environ["NEO4J_URI"]
...
gds = sessions.get_or_create(
    ...
    memory=memory,
    db_connection=DbmsConnectionInfo(db_address, db_user, db_pw),

    # Because we used self-managed DB we need to specify the cloud location.
