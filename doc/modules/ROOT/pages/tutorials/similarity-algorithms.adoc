...
[source, python, role=no-test]
----
%pip install "graphdatascience>2.0" python-dotenv
----
...
[source, python, role=no-test]
----
from graphdatascience.session import CloudLocation, DbmsConnectionInfo, SessionMemory
...
After the import, we use
https://neo4j.com/docs/graph-data-science-client/current/transactions/#coordinate-parallel-transactions[bookmarks]
to ensure that the remote graph projections only execute once all
imported data is fully propagated across the database cluster.
