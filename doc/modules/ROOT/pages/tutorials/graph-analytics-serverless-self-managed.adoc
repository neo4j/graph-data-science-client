...
----
%pip install "graphdatascience>=2.0a1" python-dotenv "neo4j_viz[gds]"
----
...
data_query = """
  MERGE (dan:Person   {name: 'Dan',   age: 18, experience: 63, hipster: 0})
  MERGE (annie:Person {name: 'Annie', age: 12, experience: 5, hipster: 0})
  MERGE (matt:Person  {name: 'Matt',  age: 22, experience: 42, hipster: 0})
  MERGE (jeff:Person  {name: 'Jeff',  age: 51, experience: 12, hipster: 0})
  MERGE (brie:Person  {name: 'Brie',  age: 31, experience: 6, hipster: 0})
  MERGE (elsa:Person  {name: 'Elsa',  age: 65, experience: 23, hipster: 1})
  MERGE (john:Person  {name: 'John',  age: 4, experience: 100, hipster: 0})

  MERGE (apple:Fruit  {name: 'Apple',  tropical: 0, sourness: 0.3, sweetness: 0.6})
  MERGE (banana:Fruit {name: 'Banana', tropical: 1, sourness: 0.1, sweetness: 0.9})
  MERGE (mango:Fruit  {name: 'Mango',  tropical: 1, sourness: 0.3, sweetness: 1.0})
  MERGE (plum:Fruit   {name: 'Plum',   tropical: 0, sourness: 0.5, sweetness: 0.8})

  MERGE (dan)-[:LIKES]->(apple)
  MERGE (annie)-[:LIKES]->(banana)
  MERGE (matt)-[:LIKES]->(mango)
  MERGE (jeff)-[:LIKES]->(mango)
  MERGE (brie)-[:LIKES]->(banana)
  MERGE (elsa)-[:LIKES]->(plum)
  MERGE (john)-[:LIKES]->(plum)

  MERGE (dan)-[:KNOWS]->(annie)
  MERGE (dan)-[:KNOWS]->(matt)
  MERGE (annie)-[:KNOWS]->(matt)
  MERGE (annie)-[:KNOWS]->(jeff)
  MERGE (annie)-[:KNOWS]->(brie)
  MERGE (matt)-[:KNOWS]->(brie)
  MERGE (brie)-[:KNOWS]->(elsa)
  MERGE (brie)-[:KNOWS]->(jeff)
  MERGE (john)-[:KNOWS]->(jeff)
"""

# making sure the database is actually empty
assert gds.run_cypher("MATCH (n) RETURN count(n)", retryable=True).squeeze() == 0, "Database is not empty!"

# let's now write our graph!
gds.run_cypher(data_query, retryable=True)

gds.run_cypher("MATCH (n) RETURN count(n) AS nodeCount", retryable=True)
----
...
print(f"Compute millis: {pr_result.compute_millis}")
print(f"Node properties written: {pr_result.node_properties_written}")
print(f"Centrality distribution: {pr_result.centrality_distribution}")

print("Running FastRP ...")
frp_result = gds.fast_rp.mutate(
    G,
...
print(f"Compute millis: {frp_result.compute_millis}")
# stream back the results
gds.graph.node_properties.stream(G, ["pagerank", "fastRP"], db_node_properties=["name"])
----
...
gds.graph.node_properties.write(G, ["pagerank", "fastRP"])
----
...
[source, python, role=no-test]
----
gds.louvain.write(G, write_property="louvain")
----
...
gds.run_cypher("MATCH (n:Person|Fruit) DETACH DELETE n", retryable=True)
----
