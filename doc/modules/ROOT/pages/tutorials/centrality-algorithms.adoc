...
----
gds.run_cypher(
    "UNWIND $nodes AS node MERGE (n:City {node_id: node.node_id, name: node.name, population: node.metro_pop})",
...
gds.run_cypher(
    ...
    MERGE (source)-[:HAS_FLIGHT_TO]->(target)
    """,
...
[source, python, role=no-test]
----
eigenvector_centrality_result = gds.eigenvector_centrality.mutate(
    G, max_iterations=100, mutate_property="eigenvectorCentrality"
)
----
...
[source, python, role=no-test]
----
if eigenvector_centrality_result.did_converge:
    print(
        f"The number of iterations taken by Eigenvector Centrality to run is {eigenvector_centrality_result.ran_iterations}."
    )
else:
...
[source, python, role=no-test]
----
eigenvector_centrality_result.centrality_distribution
----
...
[source, python, role=no-test]
----
gds.graph.node_properties.write(G, ["eigenvectorCentrality"])
----
...
def display_top_20_cities(centrality_measure):
    ...
    LIMIT 20
    """
    result = gds.run_cypher(query, retryable=True)
...
[source, python, role=no-test]
----
betweenness_centrality_result = gds.betweenness_centrality.mutate(G, mutate_property="betweennessCentrality")
----
...
[source, python, role=no-test]
----
betweenness_centrality_result.centrality_distribution
----
...
[source, python, role=no-test]
----
gds.graph.node_properties.write(G, ["betweennessCentrality"])
----
...
[source, python, role=no-test]
----
degree_centrality_result = gds.degree_centrality.mutate(G, mutate_property="degreeCentrality")
----
...
[source, python, role=no-test]
----
degree_centrality_result.centrality_distribution
----
...
[source, python, role=no-test]
----
gds.graph.node_properties.write(G, ["degreeCentrality"])
----
...
----
# Cleanup database
gds.run_cypher("MATCH (n:City) DETACH DELETE n", retryable=True)
----

== References

* For the network: Brendan J. Frey and Delbert Dueck. "Clustering by
passing messages between data points." Science 315.5814 (2007): 972-976.
