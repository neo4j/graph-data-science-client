...
predicted_fastrp = gds.graph.node_properties.stream(G, "predictedClass", node_labels=["UnclassifiedPaper"])
----
...
nodes = gds.util.as_nodes(predicted_fastrp.nodeId.to_list())
