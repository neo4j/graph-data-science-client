...
gds.run_cypher(load_nodes, retryable=True)
gds.run_cypher(load_relationships, retryable=True)
----
...
[source, python, role=no-test]
----
gds.run_cypher("MATCH (n) WHERE n:Paper OR n:UnclassifiedPaper DETACH DELETE n", retryable=True)
----
