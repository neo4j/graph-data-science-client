...
example to illustrate how to connect your AuraDB instance to a GDS
Session, run algorithms, and eventually write back your analytical
results to the AuraDB database. We will cover all management operations:
...
We also have to specify the size of the session. In this example we will
use the default size, but there are many options available.
...
is a simple example, but it illustrates the main steps of using GDS
Sessions.
