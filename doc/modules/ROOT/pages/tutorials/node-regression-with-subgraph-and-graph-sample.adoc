...
[source, python, role=no-test]
----
# making sure the database is empty
...
gds.run_cypher(
    ...
    MATCH (c:Chameleon)
    SET c.features_one_hot = gds.util.oneHotEncoding(f.totality, c.features)
""",
)
gds.run_cypher(
    """
...
G_animals, projection_result = gds.graph.project.native(
    "wiki_animals",
    ["Chameleon", "Squirrel", "Crocodile"],
    {"LINK": {"orientation": "UNDIRECTED"}},
    node_properties=["features_one_hot", "target"],
)
projection_result
----
...
gds.wcc.mutate(G_animals, mutate_property="wcc_component")
----
...
components = gds.graph.node_properties.stream(G_animals, "wcc_component")
...
graph_components_by_labels[str(["Crocodile"])].drop(fail_if_missing=False)
graph_components_by_labels[str(["Squirrel"])].drop(fail_if_missing=False)
...
print("=" * 25)
print(G_chameleon.degree_distribution())
----
...
----
# Now, let's construct a training pipeline!
chameleons_nr_training, _ = gds.pipeline.node_regression.create("node_regression_pipeline__Chameleons")

# We configure the splitting
chameleons_nr_training.configure_split(validation_folds=5, test_fraction=0.2)

# We add a set of model candidates
# A linear regression model with the learningRate parameter in a search space
chameleons_nr_training.add_linear_regression(
    penalty=1e-5,
    patience=3,
    tolerance=1e-5,
    min_epochs=20,
    max_epochs=500,
    learning_rate=(100, 1000),  # We let the auto-tuner find a good value
)
# Let's try a few different models
chameleons_nr_training.configure_auto_tuning(max_trials=10)

# Our input feature dimension is 3132
# We can reduce the dimension to speed up training using a FastRP node embedding
chameleons_nr_training.add_node_property(
    "fastRP",
    embedding_dimension=256,
    property_ratio=0.8,
    feature_properties=["features_one_hot"],
    mutate_property="frp_embedding",
    random_seed=420,
)
...
chameleons_nr_training.select_features("frp_embedding")
...
nc_model, train_result = chameleons_nr_training.train(
    ...
    model_name="chameleon_nr_model",
    target_node_labels=["Chameleon"],
...
[source, python, role=no-test]
----
print("Winning model parameters: \n\t\t" + str(nc_model.best_parameters()))
print()
print("MEAN_SQUARED_ERROR      test score: " + str(train_result.model_info["metrics"]["MEAN_SQUARED_ERROR"]["test"]))
print("MEAN_ABSOLUTE_ERROR     test score: " + str(train_result.model_info["metrics"]["MEAN_ABSOLUTE_ERROR"]["test"]))
----
...
G_chameleon_sample, _ = gds.graph.sample.rwr(
    G_chameleon,
    "cham_sample",
    sampling_ratio=0.30,  # We'll use 30% of the graph
)
...
nc_model_sample, train_result_sample = chameleons_nr_training.train(
    G_chameleon_sample,
    model_name="chameleon_nr_model_sample",
    target_node_labels=["Chameleon"],
    target_property="target",
    metrics=["MEAN_SQUARED_ERROR", "MEAN_ABSOLUTE_ERROR"],
    random_seed=420,
...
[source, python, role=no-test]
----
print("Winning model parameters: \n\t\t" + str(nc_model_sample.best_parameters()))
print()
print(
    "MEAN_SQUARED_ERROR      test score: "
    + str(train_result_sample.model_info["metrics"]["MEAN_SQUARED_ERROR"]["test"])
)
print(
    "MEAN_ABSOLUTE_ERROR     test score: "
    + str(train_result_sample.model_info["metrics"]["MEAN_ABSOLUTE_ERROR"]["test"])
)
----
...
real_targets = gds.graph.node_properties.stream(G_chameleon, "target")
...
display(merged_all.tail(10))
----

== Cleanup

Before finishing we clean up the example data from both the GDS
in-memory state and the database.

[source, python, role=no-test]
----
# Cleanup GDS
# Drop the in-memory graphs that are still in the Graph Catalog
G_animals.drop(fail_if_missing=False)
G_chameleon.drop(fail_if_missing=False)
G_chameleon_sample.drop(fail_if_missing=False)
----

[source, python, role=no-test]
----
# Cleanup database
# Remove the nodes we created, and drop the constraints, so the database is left empty
gds.run_cypher("MATCH (n) DETACH DELETE n")

gds.run_cypher("DROP CONSTRAINT chameleons IF EXISTS")
gds.run_cypher("DROP CONSTRAINT crocodiles IF EXISTS")
gds.run_cypher("DROP CONSTRAINT squirrels IF EXISTS")
----
