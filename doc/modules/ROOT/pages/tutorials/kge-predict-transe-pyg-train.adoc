...
def process():
    ...
    for file_name in raw_file_names:
        ...
        edges_by_rel_type[rel_types[file_name]] = list_of_dicts

    # making sure the database is empty
    assert gds.run_cypher("MATCH (n) RETURN count(n)").squeeze() == 0, "Database is not empty!"

    # Create the (unique) entity nodes once
    print(f"Writing {len(node_dict_)} entities")
    gds.run_cypher(
        "UNWIND $nodes AS n CREATE (:Entity {id: n.id, text: n.text})",
        params={"nodes": [{"id": node_id, "text": text} for text, node_id in node_dict_.items()]},
    )

    # Create the relationships, one relationship type per source file
    for rel_type, list_of_dicts in edges_by_rel_type.items():
        print(f"Writing {len(list_of_dicts)} relationships of {rel_type}")
        gds.run_cypher(
            f"""
            UNWIND $ll AS l
            MATCH (n:Entity {{id: l.source}}), (m:Entity {{id: l.target}})
            CREATE (n)-[:{rel_type} {{rel_id: l.rel_id}}]->(m)
            """,
    ...
    for rel_id, rels in tqdm(rel_type_dict.items()):
        ...
        gds.run_cypher(
            ...
            UNWIND $ll AS l
            MATCH (n:Entity {{id: l.source}}), (m:Entity {{id: l.target}})
            CREATE (n)-[:{REL_TYPE} {{rel_id: $rel_id, text: $text}}]->(m)
            """,
...
ttv_G, result = gds.graph.project.native(
    "fb15k-graph-ttv",
    node_projection,
    relationship_projection,
)

node_properties = gds.graph.node_properties.stream(ttv_G, ["id"])
...
def create_data_from_graph(relationship_type):
    rels_tmp = gds.graph.relationships.stream(ttv_G, relationship_type, ["rel_id"])
    topology = [
...
[source, python, role=no-test]
----
gds.graph.drop(ttv_G, fail_if_missing=False)
----
...
[source, python, role=no-test]
----
node_embeddings = model.node_emb.weight.detach().cpu()
num_nodes = node_embeddings.size(0)

batch_size = 1000
for start in tqdm(range(0, num_nodes, batch_size)):
    rows = [
        {"id": start + offset, "emb": emb}
        for offset, emb in enumerate(node_embeddings[start : start + batch_size].tolist())
    ]
    gds.run_cypher(
        """
        UNWIND $rows AS row
...
[source, python, role=no-test]
----
G_test, result = gds.graph.project.native(
    "graph_to_predict_",
...
transe_model = gds.kge.transe(G_test, "emb", {rel_label_to_predict: target_emb})
----
...
result = transe_model.predict_stream(
    source_node_filter=source_ids_df.nodeId.tolist(),
...
[source, python, role=no-test]
----
gds.graph.drop(G_test, fail_if_missing=False)
----

== Cleanup

Before finishing we clean up the example data from the database.

[source, python, role=no-test]
----
# Cleanup database
# Remove the entities and relationships we created, and drop the constraint,
# so the database is left empty (as it was expected to be at the start of this notebook)
gds.run_cypher("MATCH (n) DETACH DELETE n")

gds.run_cypher("DROP CONSTRAINT entity_id IF EXISTS")
----
