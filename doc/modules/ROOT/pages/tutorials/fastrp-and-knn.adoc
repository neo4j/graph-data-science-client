...
%pip install "graphdatascience>2.0a1"
----
...
# We use MERGE so the query is idempotent and can safely be retried.
_ = gds.run_cypher(
    """
        MERGE (dan:Person {name: 'Dan'})
        MERGE (annie:Person {name: 'Annie'})
        MERGE (matt:Person {name: 'Matt'})
        MERGE (jeff:Person {name: 'Jeff'})
        MERGE (brie:Person {name: 'Brie'})
        MERGE (elsa:Person {name: 'Elsa'})

        MERGE (cookies:Product {name: 'Cookies'})
        MERGE (tomatoes:Product {name: 'Tomatoes'})
        MERGE (cucumber:Product {name: 'Cucumber'})
        MERGE (celery:Product {name: 'Celery'})
        MERGE (kale:Product {name: 'Kale'})
        MERGE (milk:Product {name: 'Milk'})
        MERGE (chocolate:Product {name: 'Chocolate'})

        MERGE (dan)-[:BUYS {amount: 1.2}]->(cookies)
        MERGE (dan)-[:BUYS {amount: 3.2}]->(milk)
        MERGE (dan)-[:BUYS {amount: 2.2}]->(chocolate)

        MERGE (annie)-[:BUYS {amount: 1.2}]->(cucumber)
        MERGE (annie)-[:BUYS {amount: 3.2}]->(milk)
        MERGE (annie)-[:BUYS {amount: 3.2}]->(tomatoes)

        MERGE (matt)-[:BUYS {amount: 3}]->(tomatoes)
        MERGE (matt)-[:BUYS {amount: 2}]->(kale)
        MERGE (matt)-[:BUYS {amount: 1}]->(cucumber)

        MERGE (jeff)-[:BUYS {amount: 3}]->(cookies)
        MERGE (jeff)-[:BUYS {amount: 2}]->(milk)

        MERGE (brie)-[:BUYS {amount: 1}]->(tomatoes)
        MERGE (brie)-[:BUYS {amount: 2}]->(milk)
        MERGE (brie)-[:BUYS {amount: 2}]->(kale)
        MERGE (brie)-[:BUYS {amount: 3}]->(cucumber)
        MERGE (brie)-[:BUYS {amount: 0.3}]->(celery)

        MERGE (elsa)-[:BUYS {amount: 3}]->(chocolate)
        MERGE (elsa)-[:BUYS {amount: 3}]->(milk)
    """,
    retryable=True,
)
----
...
print(f"Required memory for native loading: {result.required_memory}")
----
...
print(f"The projection took {result.project_millis} ms")
...
print(f"Required memory for running FastRP: {result.required_memory}")
----

[source, python, role=no-test]
----
# Now let's run FastRP and mutate our projected graph 'purchases' with the results
result = gds.fast_rp.mutate(
    G,
...
print(f"Number of embedding vectors produced: {result.node_properties_written}")
----
...
print(f"Relationships produced: {result.relationships_written}")
print(f"Nodes compared: {result.nodes_compared}")
print(f"Mean similarity: {result.similarity_distribution['mean']}")
----
...
(`Product` nodes named "Cucumber" and "Tomatoes") for both people and
with similar amounts. We can therefore have high confidence in our
approach.

== Making recommendations

Using the information we derived that the `Person` nodes named "Annie"
and "Matt" are similar, we can make product recommendations for each of
them. Since they are similar, we can assume that products purchased by
only one of the people may be of interest to buy also for the other
...
_ = gds.run_cypher("MATCH (n) DETACH DELETE n", retryable=True)
----
