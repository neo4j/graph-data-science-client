...
G_sample, _ = gds.graph.sample.rwr(G, "cora_sample", random_seed=42, concurrency=1)
----
...
sample_node_properties = gds.graph.node_properties.stream(G_sample, ["subject", "features"])
----
...
[source, python, role=no-test]
----
_ = G_sample.drop(fail_if_missing=False)
