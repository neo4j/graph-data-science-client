...
found was logistic regression with `penalty=0.631876`.

Further we note that the test set accuracy score is 0.65213676, which is
actually a big improvement compared to the currently best state of the
art score in the literature. More on this in the
...
Transformer Network], in order to compare with the current state of the
...
In light of this, it is indeed very impressive that we get a test set
accuracy score of 65.21 % with HashGNN and logistic regression.
Especially considering that: - we use fewer bits to represent the
embeddings (1256 vs 2048) - use dramatically fewer training parameters
in our gradient descent compared to the deep learning models above -
HashGNN is an unsupervised algorithm - HashGNN runs a lot faster (even
without a GPU) and requires a lot less memory
