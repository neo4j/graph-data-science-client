...
the `neo4j{plus}s` protocol. The client automatically detects an AuraDS
instance and applies the AuraDS-recommended settings.
...
[source, python, role=no-test]
----
gds.graph.node_properties.stream(G, ["subject"]).head(10)
----
