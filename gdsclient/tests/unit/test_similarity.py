...
def test_similarity_stream(
    ...
    gds.alpha.similarity.cosine.stream(hello=0.2, you=42)

    assert runner.last_query() == "CALL gds.alpha.similarity.cosine.stream($config)"
    assert runner.last_params() == {
        "config": {"hello": 0.2, "you": 42},
    }
...
def test_similarity_write(runner: CollectingQueryRunner, gds: GraphDataScience) -> None:
    gds.alpha.similarity.cosine.write(hello=0.2, you=42)

    assert runner.last_query() == "CALL gds.alpha.similarity.cosine.write($config)"
    assert runner.last_params() == {
        "config": {"hello": 0.2, "you": 42},
    }
