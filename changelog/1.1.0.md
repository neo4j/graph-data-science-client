...
* Added new convenience method `NCModel.classes` to list the that was used to train a node classification model.
* Added new convenience method `model_info` to get the information of the model that was derived in training it.
* Added the following new convenience methods to link prediction, node classification and node regression model objects:
  ...
  * `pipeline` which returns information about the pipeline steps that are part of running prediction with the model.
