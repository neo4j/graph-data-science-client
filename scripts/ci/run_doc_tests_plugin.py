...
def _resolve_license_dir() -> str | None:
    ...
    if key:
        license_dir = tempfile.mkdtemp(prefix="gds_license_")
        Path(license_dir, "license_key").write_text(key)
        return license_dir

    file_path = os.environ.get("GDS_LICENSE_FILE") or os.path.expanduser("~/.gds_license")
    if os.path.isfile(file_path):
        license_dir = tempfile.mkdtemp(prefix="gds_license_")
        Path(license_dir, "license_key").write_text(Path(file_path).read_text())
        return license_dir
...
def main() -> None:
    ...
    # Enterprise scope is a superset of community, so run both (+ networkx) when licensed;
    # otherwise run only the community-safe scopes. No `-n` filter means all test methods run.
    scope_filter = None if enterprise else "/community|networkx/"
