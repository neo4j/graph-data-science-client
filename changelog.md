...
* `gds.graph.drop` now returns a list of `GraphInfo` objects as it now allows to delete multiple graphs at once.
...
* Added support for the topological link prediction functions (`gds.topological_link_prediction`) in Aura Graph Analytics sessions.
